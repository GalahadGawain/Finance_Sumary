    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            with tab1:
                ...
                st.metric("Total Expenses", f"{total_payments:,.2f} AED")
                ...
                st.subheader("Expanse Summary")
                status_totals = (
                    st.session_state.debits_df.groupby("Category")["Amount"]
                    .sum()
                    .reset_index()
                )
                status_totals = status_totals.sort_values("Amount", ascending=False)

                st.dataframe(
                    status_totals,
                    column_config={
                        "Amount": st.column_config.NumberColumn(format="%.2f AED")
                    },
                    use_container_width=True,
                    hide_index=True,
                )
            ...
            with tab2:
                ...
                st.subheader("Payments Summary")
                status_totals = (
                    st.session_state.credits_df.groupby("Status")["Amount"]
                    .sum()
                    .reset_index()
                )
                status_totals = status_totals.sort_values("Amount", ascending=False)
